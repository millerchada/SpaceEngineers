...
// the production queue. (Inventory slot reordering is unrelated — it
// repositions physical item stacks, never touches a queue.)
//
// Custom Data is the control panel. No PB argument required for normal
// operation. Diagnostics live under [IOPM.*] sections.
//
// v2.1.3: fixed balancing/routing fractionalizing whole-unit items (e.g.
// Components showing "~922x"). Only Ore/Ingot may be fractional
// (IsFractionalAllowedTypeId); everything else is floored to whole units
// at every computed-partial-amount site (TryTransferItem's capacity fit,
// BalancePools' move amount). Integral balancing now targets a whole-number
// share per container via largest-remainder distribution (e.g. 2770 across
// 3 equal containers -> 924/923/923), not a fractional split. No change to
// production/queue behavior. Older per-version notes: see prior releases.
...
// Only Ore/Ingot are legitimately mass-based/fractional; every other known
// or unknown TypeId defaults to requiring whole-unit amounts — the PB API
// doesn't expose HasIntegralAmounts, so this is IOPM's explicit safe list.
...
bool TryTransferItem(IMyInventory src, IMyInventory dest, MyInventoryItem item, MyFixedPoint requested, out MyFixedPoint moved, out bool hadRoom)
{
    ...
    if (!fullyFits)
    {
        ...
        // Never fabricate a fractional amount of a whole-unit item (e.g.
        // Components) just because it's capacity-constrained — floor to
        // the nearest whole unit instead of transferring a partial stack.
...
int BalancePools(int transferBudget)
{
    ...
    for (int g = 0; g < groupKeys.Count && transferBudget > 0; g++)
    {
        ...
        foreach (var itemKv in perItemTotal)
        {
            ...
            // Integral items (e.g. Components) get a whole-number target per
            // container via the largest-remainder method, so e.g. 2770 across
            // three equal-capacity containers converges to 924/923/923, not
            // a fractional third each. Fractional-allowed items (Ore/Ingot)
            // keep the plain capacity-weighted share, unchanged.
