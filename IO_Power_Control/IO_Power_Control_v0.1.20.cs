...
int SoftBudget() { return Runtime.MaxInstructionCount * _c.InstrBudgetPercent / 100; }
int Ceiling() {
...
// THE REPORT RUNS LAST IN THE TICK. Nothing follows it, so reserving for the tail a SECOND
// time is simply wrong. v0.1.19 applied the reduced ceiling uniformly and the production grid
// paid for it: a report that took 5 ticks under v0.1.18 took 36 ticks and 41.5 seconds, and
// Custom Data looked stale the whole time. The reservation belongs to phases that have a tail.
bool OverReport() { return Runtime.CurrentInstructionCount > SoftBudget(); }
...
bool _rHdr;                // this section has already written its header
bool _rAll;                // `scan all`
double _rStarted;
...
// ---------------------------------------------------------------------------------------
void RefreshDetailChunk() { RefreshDetailChunk(Ceiling()); }
void RefreshDetailChunk(int budget) {
...
string StartReport(bool all) {
  ...
  _scanCursor = 0;
  _rs = R_DETAIL; _rIdx = 0; _rHdr = false;
  return "scan queued - building across ticks, written to Custom Data when complete";
}

// SECTION HEADER, WRITTEN EXACTLY ONCE.
//
// Every section used to write its header under `if (_rIdx == 0)`. That is not a once-only
// guard: if a section is entered and the budget trips BEFORE the cursor advances, it returns
// with _rIdx still 0 and writes the header again on the next tick. The production report
// printed "== UNKNOWN CONSUMERS ==" twice, and a starved section could print it many times.
// A cosmetic symptom of a real bug - the guard was testing progress, not whether the header
// had been written.
void NextSec(int s) { _rs = s; _rIdx = 0; _rHdr = false; }
bool Hdr() { if (_rHdr) return false; _rHdr = true; return true; }
...
int RepDetail() {
  int before = _rDetail;
  while (!OverReport() && _rDetail < _rBi.Count) {
    RefreshDetailChunk(SoftBudget());
...
int RepHead() {
  ...
  _rb.Append("CapacityRisk=").Append(CONDN[_risk])
    ...
    .Append(_riskCand == _risk
      ? " stable for " + Fx(_now - _riskCandSince) + "s"
      : " CandidateHeld=" + Fx(_now - _riskCandSince) + "/"
        + Fx(_riskCand > _risk ? _c.RiskPromoteSeconds : _c.RiskRecoverSeconds) + "s")
    .Append("  (advisory only; debounced so ordinary production cycles do not chatter)\n");
