// IO Power Control v0.1.5 - power capacity, protection and load-shedding controller for
    ...
    if (bat != null) {
      ...
      if (pp != null) {
        ...
        _prod.Add(new PI { P = pp, Id = pp.EntityId, Con = con, Name = pp.CustomName,
          ...
          Proven = had ? was.Proven : 0, Env = env });
      }
    }
...
void Measure() {
  ...
  for (int i = 0; i < _prod.Count; i++) {
    ...
    if (working != p.WasWorking) {
      ...
      // Make it prove itself again.
      if (working) p.Proven = 0;
