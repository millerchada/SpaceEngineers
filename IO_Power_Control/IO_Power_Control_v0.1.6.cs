// IO Power Control v0.1.6 - power capacity, protection and load-shedding controller for
...
double _genCred, _reserveNameplate, _credPeak, _credFlatSince, _demandFloor;
...
void Stress() {
  // DEMAND PRESSURE. _demandFloor tracks the recent quiet level: it drops instantly to any
  // lower demand and creeps up slowly toward a higher one, so a step increase shows as a gap
  // and a steady load closes that gap within about a minute. The rise must be worth acting on
  // - at least a whole shed threshold - before it counts as pressure at all.
  if (_demand < _demandFloor || _demandFloor <= 0) _demandFloor = _demand;
  else _demandFloor += (_demand - _demandFloor) * 0.02;
  double rise = _demand - _demandFloor;

  bool batt = _battNetOut > 0.1;
  // Without this term the clause fires on any steady base, because at a cold start credible
  // equals current and reserve is structurally zero. Demand pressure is what distinguishes
  // "a load appeared that generation cannot follow" from "this station is simply busy".
  bool ceiling = _reserve < _c.ShedReserveMW && _fuelAtCeiling &&
                 _credFlatSince > 0 && _now - _credFlatSince >= _c.StressHoldSeconds &&
                 rise >= _c.ShedReserveMW;
  bool now = batt || ceiling;
  if (now != _stressed) {
    Ev(now ? ("ELECTRICAL STRESS" + (batt ? " - batteries carrying " + MW(_battNetOut)
                                          : " - generation at demonstrated ceiling"))
...
void WriteScan(bool all) {
  ...
  _sb.Append("Stressed=").Append(_stressed).Append(" CapacityRisk=").Append(CONDN[_risk])
    ...
    .Append("s DemandFloor=").Append(Fx(_demandFloor))
    .Append("MW DemandRise=").Append(Fx(_demand - _demandFloor))
    .Append("MW (pressure needs >=").Append(Fx(_c.ShedReserveMW)).Append("MW)\n");
