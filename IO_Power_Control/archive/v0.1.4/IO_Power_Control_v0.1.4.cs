// IO Power Control v0.1.4 - power capacity, protection and load-shedding controller for
...
void Tick() {
  ...
  Echo("Power Control v" + VERSION + "  " + ROLEN[_role] + "/" + MODEN[_mode] +
    "  cond " + CONDN[_cond]);
  Echo("gen " + MW(_genCur) + " / " + MW(_genMax) + "   demand " + MW(_demand) +
    "   reserve " + MW(_reserve));
...
void Measure() {
  ...
  _genCur = 0; _genMax = 0; _genProven = 0; _largestProd = 0; _largestProdName = "-";
  for (int i = 0; i < _prod.Count; i++) {
    ...
    _genCur += cur; _genMax += mx; _genProven += p.Proven;
    if (mx > _largestProd) { _largestProd = mx; _largestProdName = p.Name; }
  ...
  _reserve = _genMax - _demand;
  _n1 = _genMax - _largestProd - _demand;
  _rechargeExposure = _battMaxIn;
  _potential = 0; _known = 0; _unknown = 0; _unkDefs.Clear();
  for (int i = 0; i < _bi.Count; i++) {
    var r = _bi[i];
...
void Condition() {
  ...
  double pct = _genMax > 0 ? _reserve / _genMax * 100 : -100;
  ...
  if (_genMax <= 0 && _demand > 0 && soc < 0.10) k = K_EMERG;
  else if (_reserve < 0 && soc < 0.10) k = K_EMERG;
  if (k != _cond) {
    Ev("Power condition " + CONDN[_cond] + " -> " + CONDN[k] + " (reserve " + MW(_reserve) + ")");
    _cond = k;
  }
}
...
// ---------------------------------------------------------------------------------------
void RestoreStep() {
  if (_reserve >= _c.RestoreReserveMW) {
...
void Sample() {
  ...
  _lastHist = _now;
  _hist.Add(new SAMP { T = (float)_now, GenCur = (float)_genCur, GenMax = (float)_genMax,
...
string PageMain() {
  ...
  Row("Condition", CONDN[_cond]);
  _sb.Append("\nGENERATION\n");
  Row(" Current", MW(_genCur));
  Row(" Available", MW(_genMax) + (_countBatt ? "+b" : ""));
  // Rated vs witnessed. On a fuel-fed base these diverge hard, and the gap is the warning.
  Row(" Proven", MW(_genProven));
  Row(" Utilization", _genMax > 0 ? Pct(_genCur / _genMax) : "-");
  _sb.Append("\nDEMAND\n");
  Row(" Current", MW(_demand));
  Row(" Potential", MW(_potential));
  Row(" Model Coverage", Pct(Coverage()));
  _sb.Append("\nRESERVE\n");
  Row(" Current", MW(_reserve));
  Row(" N-1", MW(_n1));
  ...
  // A producer that has never been seen to deliver anything near its rating may simply not
  // have been asked - or may be unable to. The script cannot tell which, and says so rather
  // than picking the comfortable interpretation.
  // NET, not gross: a battery in Auto reports a gross output continuously, so testing gross
  // here would have latched this warning on permanently and taught the operator to ignore it.
  if (_genMax > 0 && _genProven < _genMax * 0.5 && _battNetOut > 0.1)
    _sb.Append("\nWARNING\n rated ").Append(MW(_genMax)).Append(" but only\n ")
      .Append(MW(_genProven)).Append(" ever delivered,\n and batteries are\n discharging now\n");
  if (Coverage() < 0.98)
...
void WriteScan(bool all) {
  ...
  for (int i = 0; i < _prod.Count; i++) {
    ...
    _sb.Append(p.P.IsWorking ? "ON  " : "OFF ").Append(Trim(p.Name, 28)).Append("  ")
      ...
      .Append(Fx(p.P.MaxOutput)).Append("MW  proven=").Append(Fx(p.Proven)).Append("MW  grid=")
