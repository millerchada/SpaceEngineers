// IO Power Control v0.1.12 - power capacity, protection and load-shedding controller for
...
void ShedStep() {
  ...
  if (_shedActions > 0) {
    // RE-AUTHORISATION FROM THE LIVE CONDITION. Stressed stays latched through the recovery
    // hold by design; using it here would let a slow-clearing alarm authorise a new action
    // every second long after the deficit had gone.
    if (_battNetOut < _battBar) {
      if (_shedPhase != SP_STOPPED) {
        ...
        Ev("SHEDDING STOPPED - drain " + MW(_battNetOut) + " below bar " + MW(_battBar) +
           " after " + _shedActions + " action(s); stress latch stays for recovery hysteresis");
      }
      return;
    }
    if (_shedPhase != SP_ARMED) {
      _shedPhase = SP_ARMED;
...
void WriteScan(bool all) {
  ...
  _sb.Append("ShedPhase=").Append(SPN[_shedPhase])
    ...
    .Append("MW  (continuation is re-authorised from the LIVE drain against the bar, never "
      + "from the latched Stressed flag)\n");
