// IO Power Control v0.1.7 - power capacity, protection and load-shedding controller for
...
class Cfg {
  ...
  // ...and stored energy must have fallen by at least this share of total capacity over the
  // same interval, which is what a control oscillation can never do.
  public double StoredDeclinePct = 0.5;
...
// were recording.
struct FAST {
  public float T, Demand, GenCur, GenCred, BattNet, Stored;
  public short Brown;
  public bool Str;
}
...
double _battNetOut;   // summed per-battery NET discharge; never gross - see Measure
bool _countBatt;
...
void ParseConfig() {
  ...
  c.StoredDeclinePct = _ini.Get(SEC, "StoredDeclinePercent").ToDouble(c.StoredDeclinePct);
...
void Discover() {
  ...
  for (int i = 0; i < _all.Count; i++) {
    ...
    ReadBlockConfig(r);
    BI old;
    if (r.Src == SRC_NONE && _prev.TryGetValue(r.Id, out old) && old.Src != SRC_NONE) {
...
void Measure() {
  ...
  _potential = 0; _known = 0; _unknown = 0; _unkDefs.Clear(); _brownout = 0;
  for (int i = 0; i < _bi.Count; i++) {
    var r = _bi[i];
    ...
    if (r.F.Enabled && r.B.IsFunctional && !r.B.IsWorking) _brownout++;
...
void Stress() {
  ...
  // The timer must be CONTINUOUS. Restarting it whenever the draw dips below the bar is the
  // point: a battery oscillating across the threshold is not draining, and an instantaneous
  // test cannot tell the two apart.
  if (!material) _battStressSince = -1e9;
  else if (_battStressSince < -1e8) { _battStressSince = _now; _battStoredAtStress = _battStored; }
  bool sustained = material && _battStressSince > -1e8 &&
                   _now - _battStressSince >= _c.StressHoldSeconds;
  _battDecline = sustained ? _battStoredAtStress - _battStored : 0;
  double needFall = _battMaxStored * _c.StoredDeclinePct / 100;
  // Stored energy must ACTUALLY have gone down. This is what rejects the control oscillation:
  // a bank reporting 11 MW out against 12 MW in moves no net energy, so nothing falls.
  bool now = sustained && _battDecline >= needFall;
  if (now != _stressed) {
    Ev(now ? ("ELECTRICAL STRESS - batteries draining " + MW(_battNetOut) + " for " +
              Fx(_now - _battStressSince) + "s, stored down " + Fx(_battDecline) + " MWh")
           : "Electrical stress cleared");
    _stressed = now;
  }
}
...
void ShedStep() {
  if (_reserve >= _c.ShedReserveMW) return;
...
bool ShedOne(BI r, double relief) {
  ...
  if (b == null || !b.IsFunctional || IsProtectedNow(b) || r.CfgProt) return false;
  var bat = b as IMyBatteryBlock;
  if (bat != null) {
    if (bat.ChargeMode != ChargeMode.Recharge) return false;
  ...
  if (r.F == null || !r.F.Enabled) return false;
...
void SampleFast() {
  ...
  _fast.Add(new FAST { T = (float)_now, Demand = (float)_demand, GenCur = (float)_genCur,
    GenCred = (float)_genCred, BattNet = (float)_battNetOut, Stored = (float)_battStored,
...
void Sample() {
  ...
  _hist.Add(new SAMP { T = (float)_now, GenCur = (float)_genCur, GenMax = (float)_genCred,
    ...
    Net = (float)(_battIn - _battOut), Pot = (float)_potential, Cond = (byte)_cond });
...
string PageMain() {
  ...
  Row(" Net Flow", MW(_battIn - _battOut));
...
void WriteScan(bool all) {
  ...
  _sb.Append("BattGrossOut=").Append(Fx(_battOut)).Append("MW BattGrossIn=").Append(Fx(_battIn))
    .Append("MW BattNetOut=").Append(Fx(_battNetOut))
  ...
  _sb.Append("BattStressBar=").Append(Fx(_battBar)).Append("MW  held=")
    ...
    .Append("/").Append(Fx(_battMaxStored * _c.StoredDeclinePct / 100)).Append("MWh needed\n");
  ...
  _sb.Append("count=").Append(_brownout).Append('\n');

  _sb.Append("\n== FAST RING (1 Hz, newest last) ==\n");
  _sb.Append("age    dem   gcur  gcred bnet  stored br s\n");
  for (int i = 0; i < _fast.Count; i++) {
    ...
    _sb.Append(PadL(Fx(_now - f.T), 5)).Append(' ').Append(PadL(Fx(f.Demand), 6)).Append(' ')
      ...
      .Append(PadL(Fx(f.BattNet), 5)).Append(' ').Append(PadL(Fx(f.Stored), 6)).Append(' ')
  ...
  _sb.Append("age    gcur  gcred dem   rsv   stored net   cond\n");
