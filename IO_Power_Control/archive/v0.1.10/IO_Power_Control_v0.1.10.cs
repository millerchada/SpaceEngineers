// IO Power Control v0.1.10 - power capacity, protection and load-shedding controller for
...
void Stress() {
  ...
  if (!_stressed) {
    ...
    // Stored energy must ACTUALLY have gone down. This is what rejects the control
    // oscillation: a bank reporting 11 MW out against 12 MW in moves no net energy.
    if (sustained && _battDecline >= _battMaxStored * _c.StoredDeclinePct / 100) {
      _stressed = true;
      _recoverSince = -1e9;
