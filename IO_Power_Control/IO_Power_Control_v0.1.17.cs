...
// The COOPERATIVE soft ceiling. Deliberately a fraction of the hard limit, so a phase yields
// with room for whatever else the tick still has to do rather than stopping at the cliff edge.
bool Over() {
  return Runtime.CurrentInstructionCount >
         Runtime.MaxInstructionCount * _c.InstrBudgetPercent / 100;
}
...
void Tick() {
  ...
  if (_tickLast > _tickPeak) _tickPeak = _tickLast;
...
void WriteScan(bool all) {
  // Deliberately does NOT force a discovery pass. The model is maintained continuously, and a
  // synchronous full rediscovery is precisely the unbounded work that terminated this script
  // on the production grid - a diagnostic command must not be able to kill the thing it is
  // diagnosing. `rescan` queues a staged pass instead.
  //
  // The detail top-up keeps its wider budget so the report is as complete as it can be, but it
  // is still a budget: on a large construct the report says how much it managed.
  int save = _c.ScanChunk, saveB = _c.InstrBudgetPercent;
  _c.ScanChunk = _bi.Count + 1; _c.InstrBudgetPercent = 90;
  _scanCursor = 0;
  RefreshDetailChunk();
  ...
  _sb.Append("\n== RUNTIME COST ==\n");
  _sb.Append("hard limit ").Append(Runtime.MaxInstructionCount)
  ...
  _sb.Append("constructor ").Append(_ctorCost)
    .Append("   last full tick ").Append(_tickLast)
    .Append("   PEAK TICK ").Append(_tickPeak).Append('\n');
  _sb.Append("boot ").Append(BN[_boot]).Append("   discovery ").Append(DN[_ds])
    .Append("   modelPublished=").Append(_modelReady)
    .Append("   lastPhaseYieldedOnBudget=").Append(_yielded).Append('\n');
  _sb.Append("phase            last   peak   work\n");
  for (int i = 0; i < NPH; i++)
    _sb.Append(PadR(PHN[i], 16)).Append(PadL(_phLast[i].ToString(), 6)).Append(' ')
      .Append(PadL(_phPeak[i].ToString(), 6)).Append(' ')
      .Append(PadL(_phWork[i].ToString(), 6)).Append('\n');
  _sb.Append("discovery ").Append(_dProcessed).Append("/").Append(_dTotal)
    .Append("   PEAK TICK against the hard limit is the number that matters\n");

  _sb.Append("\n== CANDIDATE AUDIT (last shed action) ==\n");
  if (_auditAt < 0) _sb.Append("(no shed action has run yet)\n");
  else {
    _sb.Append("age=").Append(Fx(_now - _auditAt)).Append("s  considered=").Append(_audit.Count)
      .Append("  shed=").Append(_auditShed).Append('\n');
    _sb.Append("curIn/hasCur/relief are the CACHED values the filter used. 'fresh' lines are a\n");
    _sb.Append("read taken DURING the audit, after the decision - evidence about the block, not\n");
    _sb.Append("about what the filter saw. They are never written back.\n");
    _sb.Append("name                      cat         tier    en cur    curIn  maxIn  relief verdict\n");
    for (int i = 0; i < _audit.Count; i++) {
      var a = _audit[i];
      _sb.Append(PadR(a.Name, 25)).Append(' ').Append(PadR(CATN[a.Cat], 11)).Append(' ')
      ...
      if (a.V == CV_RELIEF) {
        _sb.Append("    fresh parse: ").Append(a.FreshOk ? "ok" : "NO INPUT LINE FOUND")
          ...
          .Append('\n');
        _sb.Append("    fresh detail: ").Append(a.Detail).Append('\n');
      }
    }
  }
