"""Generate docs/industrial-overhaul/v1.7.7/ from machine_evidence/*.json.

    python tests/gen_io_docs.py
...
EV = os.path.join(ROOT, 'machine_evidence')
DOC = os.path.join(ROOT, 'docs', 'industrial-overhaul', 'v1.7.7')
...
def main():
    ...
    w = ['# Industrial Overhaul v1.7.7 — live production evidence', '',
         'GENERATED from `machine_evidence/*.json` by `tests/gen_io_docs.py`.',
    ...
    w = ['# Production machines — IO v1.7.7', '',
         'GENERATED from `machine_evidence/*.json`. Do not hand-edit.', '',
    ...
    w += ['', 'Reconciliation against IOPM (ItemDefs, recipes, StockConfigurable,',
          ...
          '`python tests/audit_machine_coverage.py <source.cs>` rather than frozen here,',
