...
Ingested the operator's consolidated live IO v1.7.7 handoff: **15 machines
ENUMERATION COMPLETE, 137 machine-recipe entries, Survival Kit DEFERRED**. The
handoff is AUTHORITATIVE over historical or public recipe data wherever they
conflict.

### Two corrections applied

    SolarCell   IronIngot 2 -> 3
    Gunpowder   1 | Munitions Factory | PotassiumNitrate:6, Carbon:2, Sulfur:2

SolarCell is the first defect any audit has found in a recipe that was ALREADY
RUNNING. Machine coverage caught it by comparing the shipped recipe against live
evidence - neither catalog integrity nor closure could ever have seen it, because
both start from what IOPM already believes.

Gunpowder closes the last MANUFACTURED_MISSING_RECIPE leaf.
**NO BLUEPRINT ID WAS INVENTED.** None is known, so `TryGetBlueprint` fails and
the row reports `Action=UnknownBlueprint` - a loud designed diagnostic. That is
deliberately better than guessing: a fabricated MyDefinitionId parses fine, fails
`CanUseBlueprint` on every machine, and reports `NoMachine` instead, pointing at
the hardware rather than at the catalog. `[BlueprintOverrides] Gunpowder=<id>`
closes it with no code change once the id is known.

### Audit state after the corrections

    catalog integrity   PASS   45 products / 47 recipes / 0 pending
    dependency closure  COMPLETE   16 leaves, all explicit process boundaries
    machine coverage    MANAGED 48 | IDENTITY_ONLY 12 | MISSING 74 | UNKNOWN 3
                        ingredient mismatches: none

CLOSURE COMPLETE IS NOT CATALOG COMPLETE. The graph now terminates properly, and
machine coverage still reports 74 products the mod exposes that IOPM does not
model. Counting roots hid SyntheticFabric and Gunpowder for three versions;
counting closure hides everything the graph never references. Three different
questions, three different audits, and only the third can see absence.

### Evidence and generated docs

`machine_evidence/` now holds 16 files, one per block, never merged. Several
outputs appear on multiple machines (Hydrogen/Oxygen Bottle on three) - a fact
about the mod, not a reason to share a file.

`docs/industrial-overhaul/v1.7.7/` is GENERATED by `tests/gen_io_docs.py` from
that evidence and must not be hand-edited: a hand-edited doc becomes a second
source of truth, which is the failure this audit stack exists to prevent.

### Multi-output architecture gap, recorded not modelled

Nuclear Reprocessing consumes SpentFuel 10, Ice 5, Sulfur 2, PotassiumNitrate 3
and yields Nuclear Fuel 3 + Depleted Uranium 7 + Nuclear Waste 10 simultaneously.
IOPM models one blueprint -> one product. Forcing it in would make the planner
re-run the recipe because only one coproduct was ever counted against demand.
Reported by the audit as an architecture gap and deliberately not modelled.

### Nothing was added to [Stock]

45 stock-configurable products, unchanged. Machine coverage and stock policy are
separate decisions and the scope proposals are open for approval.
