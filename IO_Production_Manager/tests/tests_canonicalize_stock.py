...
# Live UAT dump, kept under <project>/uat/ rather than beside production source.
# Resolved from __file__ so the suite runs from any working directory.
LIVE = io.open(os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))),
                            'uat', 'live_custom_data.txt'), encoding='utf-8').read()
