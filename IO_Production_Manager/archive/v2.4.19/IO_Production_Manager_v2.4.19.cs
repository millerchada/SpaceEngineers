// IO Production Manager v2.4.19 - warehouse sorting, production planning, stall recovery,
...
// v2.4.19: alphabetical organization MERGES split stacks of the same item before reordering,
// and can be turned off with [Sorting] Organize=false.
const string VERSION = "2.4.19";
...
int RunSorting(int tb) {
  ...
  if (_cfg.OrganizeEnabled && tb > 0) tb = OrganizeInventories(tb);
  return tb;
}
