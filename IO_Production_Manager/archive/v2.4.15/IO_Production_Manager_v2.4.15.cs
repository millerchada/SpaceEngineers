// IO Production Manager v2.4.15 - warehouse sorting, production planning, stall recovery,
...
// v2.4.15: a [No Sorting] LOCAL connector no longer stops its own inventory being cleaned
// (docking-T pattern); [Ignore] short tag; stock display tag renamed off "Stock".
const string VERSION = "2.4.15";
...
void Discover() {
  ...
  for (int i = 0; i < all.Count; i++) {
    ...
    if (lc != null) {
      ...
      // IGNORE_TAGS below.
      if (linv != null && _cfg.UnloadConnInv && !AnyTag(b.CustomName, LOCK_TAGS))
