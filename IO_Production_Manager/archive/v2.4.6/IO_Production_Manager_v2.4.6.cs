// IO Production Manager v2.4.6 - warehouse sorting, production planning, stall recovery,
...
// v2.4.6: seeds an INERT quota template into a [Stock] loadout container that has EMPTY
// Custom Data, so quotas can be filled in on the block. Existing data is never altered.
...
// Inert starting template for an empty [Stock] container: every managed component at "0M".
...
string LoadoutTemplate() {
  List<string> a = new List<string>();
  foreach (var kv in _recipes) a.Add(kv.Key);
...
void WriteDiagnostics() {
  ...
  foreach (var kv in _planRecords) {
    ...
    ini.Set(sec, "BlockedBy", IsBlockedAction(r.Action) ? r.BlockedBy : "");
