// IO Production Manager v2.3.2 — warehouse sorting/balancing/organization,
...
// live under [IOPM.*]. (2.3.2: fixed PB instruction-limit crash — status
// and stock LCD rebuilds no longer run every Update10 tick or share an
// invocation with the heavy accounting cycle; they're cached and spread
// across the two ticks following a cycle. Added [IOPM.Runtime] instruction
// counters. 2.3.1: fixed PB crash in [IOPM-Stock] display — sort
// comparator no longer calls Friendly()/dictionary lookups inline; rows
// are precomputed once with per-row error isolation, matching the
// OrganizeInventories() defensive-sort pattern. 2.3.0: added live
// [IOPM-Stock] LCD display — Item/Qty/Quota/Queued, read-only,
// auto-populates missing [Stock] aliases at 0. 2.2.0: fixed
// ReserveSupportBudget over-reserving physical stock already covered by
// queued/planned output. See prior releases for history.)
...
string _lastPhase = "";
// Presentation caches — rebuilt only when the accounting cycle actually
// refreshes their source data, never on every Update10 tick.
string _cachedStatusText = "";
string _cachedStockText = "";
bool _statusRenderPending = false;
bool _stockRenderPending = false;
...
public void Main(string argument, UpdateType updateSource)
{
    ...
    bool heavyCycleRan = false;
    if (_cfg.GeneralEnabled && _elapsedSinceCycle >= Math.Max(1, _cfg.UpdateSeconds))
    {
        DateTime start = DateTime.UtcNow;
        _lastPhase = "HeavyCycle";
        RunCycle();
        _lastCycleSeconds = (DateTime.UtcNow - start).TotalSeconds;
        _elapsedSinceCycle = 0;
        heavyCycleRan = true;
    }
    // Presentation (status/stock LCD rebuild + write) is non-critical and is
    // spread across the Update10 ticks that FOLLOW a heavy cycle, never the
    // same invocation as one — that coincidence is what exceeded the PB's
    // 50000-instruction runtime limit. Screens keep last-written content on
    // ticks where nothing is pending; that's a no-op, not a regression.
    if (!heavyCycleRan)
    {
        if (_statusRenderPending)
        {
            _lastPhase = "StatusRender";
            RenderStatusScreens();
            _statusRenderPending = false;
        }
        else if (_stockRenderPending)
        {
            _lastPhase = "StockRender";
            RenderStockScreens();
            _stockRenderPending = false;
        }
    }
    _lastInstructions = Runtime.CurrentInstructionCount;
    if (_lastInstructions > _peakInstructions) _peakInstructions = _lastInstructions;
}
void RunCycle()
{
    _cycleLog.Clear();
    ...
    WriteDiagnostics();
    // 11. Status/stock LCD rebuilds are deferred to the next two Update10
    // ticks (see Main()), never rendered inside the heavy cycle itself.
    _statusRenderPending = true;
    _stockRenderPending = true;
}
...
int OrganizeInventories(int transferBudget)
{
    ...
    for (int c = 0; c < _allContainers.Count && transferBudget > 0; c++)
    {
        ...
        for (int k = 0; k < items.Count; k++)
        {
            try
            {
                catRank[k] = multi ? CategoryRank(items[k].Type.TypeId) : 0;
...
void WriteDiagnostics()
{
    ...
    if (curInstr > _peakInstructions) _peakInstructions = curInstr;
    ini.Set("IOPM.Runtime", "LastInstructions", curInstr);
