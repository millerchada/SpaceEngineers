// IO Production Manager v2.4.4 - warehouse sorting, production planning, stall recovery,
...
// v2.4.4: display only - [Display] font control (ManageFonts=false leaves the panel's own
// Font/FontSize alone), optional stock paging, BlockedBy shown only when actually blocked.
...
void WriteDiagnostics() {
  ...
  foreach (var kv in _planRecords) {
    ...
    if (r.BlockedBy != "" && IsBlockedAction(r.Action)) ini.Set(sec, "BlockedBy", r.BlockedBy);
