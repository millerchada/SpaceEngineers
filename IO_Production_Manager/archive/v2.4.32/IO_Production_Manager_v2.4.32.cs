// IO Production Manager v2.4.32 - warehouse sorting, production planning, stall recovery,
...
// v2.4.32: [Stock] keys are also RE-SPELLED to their canonical alias. A collision is
// DETECTED AND REPORTED, never resolved by discarding a value.
const string VERSION = "2.4.32";
...
void DetectStockCollisions(MyIni ini) {
  ...
  List<MyIniKey> keys = new List<MyIniKey>();
  ini.GetKeys("Stock", keys);
  // Gather full groups, then emit SORTED. The message must not depend on the order the keys
  // happen to appear in: this pass reorders the section, so a discovery-order message would
  // read "Computer and BasicComputer" on the cycle that finds the collision and "BasicComputer
  // and Computer" on the next, making the diagnostics churn for one extra cycle. Caught by
  // tests_canonicalize_stock.py. Sorting both the group names and the groups themselves makes
  // the message a pure function of the key SET.
  ...
  for (int i = 0; i < keys.Count; i++) {
    ...
    List<string> g;
    if (!groups.TryGetValue(c, out g)) { g = new List<string>(); groups[c] = g; order.Add(c); }
    g.Add(n);
  }
  order.Sort((a, b) => string.Compare(a, b, OIC));
  for (int i = 0; i < order.Count; i++) {
    List<string> g = groups[order[i]];
    if (g.Count < 2) continue;
    g.Sort((a, b) => string.Compare(a, b, OIC));
    _stockCollide.Add(order[i]);
    _stockCollisions.Add("[Stock] alias collision on " + order[i] + ": " +
      string.Join(", ", g.ToArray()) + " all mean " + order[i] + ". All kept verbatim and NONE " +
      "renamed - delete whichever you do not want. Until then the config load order decides " +
      "which value wins.");
  }
}
