// IO Production Manager v2.3.0 — warehouse sorting/balancing/organization,
...
// live under [IOPM.*]. (2.3.0: added live [IOPM-Stock] LCD display —
// Item/Qty/Quota/Queued, read-only, auto-populates missing [Stock]
// aliases at 0. 2.2.0: fixed ReserveSupportBudget over-reserving
// physical stock already covered by queued/planned output. See prior
...
string BuildStockText()
{
    ...
    List<string> aliases = new List<string>(_recipes.Keys);
    aliases.Sort((a, b) => string.Compare(Friendly(a), Friendly(b), StringComparison.OrdinalIgnoreCase));
    sb.Append(PadRight("Item", 22)).Append(PadRight("Qty", 10)).Append(PadRight("Quota", 10)).Append("Queued\n");
    for (int i = 0; i < aliases.Count; i++)
    {
        string alias = aliases[i];
        double qty = Get(_onHand, alias);
        double quota = _cfg.StockTargets.ContainsKey(alias) ? _cfg.StockTargets[alias] : 0;
        double queued = Get(_queuedOutput, alias);
        sb.Append(PadRight(Friendly(alias), 22)).Append(PadRight(F(qty), 10)).Append(PadRight(F(quota), 10)).Append(F(queued)).Append("\n");
    }
    return sb.ToString();
}
