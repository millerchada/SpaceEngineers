// IO Production Manager v2.4.8 - warehouse sorting, production planning, stall recovery,
...
// v2.4.8: the seeded loadout template also lists what the container already holds, so
// ores/ammo/modded items appear instead of only the 25 managed components.
const string VERSION = "2.4.8";
...
// Inert starting template for an empty [Stock] container: every managed component PLUS
// whatever the container already holds, so ores, ammo and modded items are listed instead of
// only the 25 recipe items. An observed item is named by its IOPM alias when it has one, else
// by its bare SubtypeId - which resolves through the observed-subtype path (proven with Ice).
...
string LoadoutTemplate(IMyInventory inv) {
  ...
  HashSet<string> seen = HS();
  foreach (var kv in _recipes) if (seen.Add(kv.Key)) a.Add(kv.Key);
  if (inv != null) {
    // Deliberately a LOCAL list, not a shared scratch buffer: this runs inside DockDiscover
    // and must not alias a buffer another pass is holding.
    List<MyInventoryItem> its = new List<MyInventoryItem>();
    inv.GetItems(its);
    for (int i = 0; i < its.Count; i++) {
      MyItemType t = its[i].Type;
      if (IsProcessItem(t)) continue;
      string alias = AliasFromRawType(RawItemType(t));
      string nm = alias != null ? alias : t.SubtypeId.ToString();
      if (!string.IsNullOrEmpty(nm) && seen.Add(nm)) a.Add(nm);
    }
  }
