// IO Production Manager v2.4.0 - warehouse sorting/balancing/organization,
// production planning, stall recovery, status + Custom-Data diagnostics.
// ABSOLUTE INVARIANT: AddQueueItem() is the ONLY production-queue mutation in
// this file. No ClearQueue, no removal, no reorder. Custom Data is the control
// panel; the script owns [IOPM.*] only, everything else is preserved verbatim.
// v2.4.0: config snapshot frozen per logical cycle (_cfgDirty) so ApplyPlan can
// never run on a stale/mixed config; blueprint knowledge consolidated; size/alloc
// refactor. See CHANGELOG.md for 2.2.0-2.3.4 history.
...
static readonly string[] PN = new string[] { "Idle", "Discover", "Sorting", "ScanInventory",
  "StallRecovery", "BuildPlan", "ApplyPlan", "WriteDiagnostics", "StatusRender", "StockRender" };
// GOAT category vocabulary in canonical display/packing order (Overflow is our
// extension, listed last). Also the container-tag match vocabulary.
...
// Config: Custom Data is the source of truth, re-read only while idle (_cfgDirty).
class Config
{
...
// Set when Custom Data changes mid-cycle; the reload is deferred to PHASE_IDLE so
// every phase of one logical cycle sees exactly one immutable Config instance.
...
// Name -> blueprint id. Superset of the recipe-managed aliases: knowing a blueprint
// id does NOT make an item manufacturable or [Stock]-managed.
...
// PhysicalObject/Tools but is Consumables). PhysicalObject->Tools stays untouched.
...
// Deduped container list, once each. Accounting MUST iterate this, never _pools (multi-tag double-count).
...
// Physical stock reserved for existing-queue support this cycle; a later [Stock]
// branch can never spend the same material.
...
// Cooperative-cycle state: next phase, plus budgets carried Sorting -> StallRecovery.
int _cyclePhase = PHASE_IDLE;
int _cycleRecoveryReserve = 0;
int _cycleSortingLeftover = 0;
DateTime _cycleStartUtc;
// PB runtime-instruction diagnostics. Peak persists for the whole in-memory session.
int _lastInstructions = 0;
int _peakInstructions = 0;
string _lastPhase = "";
string _peakPhaseName = "";
// Set when Me.CustomData fails to re-parse; surfaced via Echo, since that is exactly
// when [IOPM.*] can't be trusted.
...
// Reused scratch buffers - hot paths must not allocate per item/container. _itemsB is
// only for BalancePools' source re-read, live at the same time as an _itemsA result.
...
// Stall detection / recovery state (in-memory only; resets on PB recompile)
class MH
{
...
public void Main(string argument, UpdateType updateSource)
{
  ...
  // Mid-cycle edits set _cfgDirty and are held until the cycle completes, so
  // Discover/Scan/BuildPlan/ApplyPlan always run against one immutable Config and
  // ApplyPlan can never commit a plan built under a different config.
  ...
  // UpdateSeconds gates only the START of a cycle; its phases then run on consecutive
  // ticks, and a second cycle can never start while one is in progress.
...
// Exactly one phase per Update10 invocation; each advances _cyclePhase. Bodies are the
// original RunCycle() logic, only split across ticks.
void RunCyclePhase()
{
  switch (_cyclePhase)
  {
...
void LoadConfig()
{
  ...
  // Production defaults to false when absent — safe fresh-install default.
  ...
  for (int i = 0; i < keys.Count; i++)
  {
    ...
    // Knowledge-only blueprint names are not items and are rejected here.
  ...
  keys.Clear();
  // [BlueprintOverrides] wins over the built-in catalog — see TryGetBlueprint().
...
void Discover()
{
  ...
  for (int i = 0; i < all.Count; i++)
  {
    ...
    // Warehouse discovery is restricted to cargo containers on this construct - O2/H2
    // generators etc. are never touched even if tagged.
    ...
    // Overflow is exclusive: a container tagged "Ores Overflow" is Overflow-only.
...
void ClassifyProductionBlock(IMyProductionBlock pb)
{
  ...
  // Disassembly is manual player work: excluded from queue scanning, production,
  // stall detection and input evacuation.
...
// Process-resource SubtypeIds (e.g. Heat): excluded from the warehouse system entirely.
...
// Only Ore/Ingot are fractional; all else integral (no HasIntegralAmounts in the PB API).
...
// Precedence: process-item exclusion (caller) -> subtype override -> TypeId map. ALL
// category lookups on a live MyItemType must come through here.
...
// Precomputed reverse index (built by AddItem) — never a linear scan per item.
...
// CanItemsBeAdded is binary-searched for the largest fitting amount. hadRoom true with a
// false result means capacity said yes but the transfer still failed.
...
// Ranked destinations: single-category (most free first) then multi-category, so callers
// can fail over. Returns a REUSED buffer - consume it before calling again.
...
// Tries each ranked candidate until one accepts; categoryExists/HasRoom are reported
// separately from Overflow's own state.
bool TryRouteToCategory(IMyInventory src, MyInventoryItem item, string category, out MyFixedPoint moved, out CI usedDest, out bool categoryExists, out bool categoryHasRoom)
{
  moved = (MyFixedPoint)0;
  usedDest = null;
  ...
  for (int i = 0; i < candidates.Count; i++)
  {
    ...
    if (TryTransferItem(src, dest.Inventory, item, item.Amount, out m, out hadRoom) && m > 0)
...
bool RouteItem(IMyInventory src, MyInventoryItem item, string category, out MyFixedPoint moved)
{
  ...
  {
    CI overflowDest;
    overflowOk = TryRouteToCategory(src, item, OVF, out moved, out overflowDest, out overflowExists, out overflowHasRoom);
  }
  if (overflowOk)
  {
...
int RunSorting(int tb)
{
  ...
  // Deduped _cons, processed once; Overflow excluded (DrainOverflow is its only exit path).
  _catInvs.Clear();
...
// One move per container per cycle (indexes go stale after).
int OrganizeInventories(int tb)
{
  ...
  for (int c = 0; c < _cons.Count && tb > 0; c++)
  {
    ...
    // Sort keys precomputed ONCE outside the comparator; lookup failures caught HERE.
    int n = items.Count;
...
// ownCats true for category containers, so items already in one of their own categories
// are left alone.
...
// Balance only within the exact same category set, so a multi-tag container's capacity
// is never counted into two pools at once.
...
int BalancePools(int tb)
{
  ...
  for (int g = 0; g < groupKeys.Count && tb > 0; g++)
  {
    ...
    foreach (var itemKv in perItemTotal)
    {
      ...
      // Integral items get whole-number targets via largest-remainder (2770/3 -> 924/923/923);
      // Ore/Ingot keep the fractional capacity-weighted share.
...
int DrainOverflow(int tb)
{
  ...
  for (int i = 0; i < overflowPool.Count && tb > 0; i++)
  {
    ...
    for (int k = 0; k < _itemsA.Count && tb > 0; k++)
    {
      ...
      if (TryRouteToCategory(of.Inventory, _itemsA[k], category, out moved, out dest, out exists, out hasRoom))
      {
        tb--;
...
void ScanInventory()
{
  ...
  _stagedStock.Clear();
  _onHand.Clear();
  // Iterate the DEDUPED list once per physical inventory - counting via _pools would
  // credit multi-tag contents twice.
  for (int i = 0; i < _cons.Count; i++)
  {
    CI ci = _cons[i];
    if (_ignInv.Contains(ci.Inventory)) continue;
  ...
  // Warehouse+Overflow+MachineOutput are finished inventory; staged inputs never satisfy [Stock].
...
// Queue scan (read-only). Reverse map cached; rebuilt only when config reloads.
...
bool RunStallRecovery(int tb)
{
  ...
  for (int i = 0; i < _mach.Count; i++)
  {
    ...
    {
      h.CooldownRemaining--;
      // Cooldown expired: clear Recovering; StallCycles resets (fresh window required).
    ...
    // IsProducing means working right now - a long recipe can run many cycles before the
    // queue head decrements; that is not a stall.
...
void BuildPlan()
{
  ...
  _planOrder.Clear();
  _planRecords.Clear();
  // Budget = on-hand surplus above the [Stock] floor. Priority 1: existing queued jobs
  // demand their ingredients; the queued item itself is untouched.
  ...
  for (int r = 0; r < queueRoots.Count; r++)
  {
    ...
    for (int i = 0; i < recipe.Inputs.Count; i++)
    {
      ...
      // EnsureFeasible already records support demand when isSupportBranch; don't double-count.
      EnsureFeasible(dep, ing.Amount * jobs, true, HS(), 0);
      // Reserve only here (once per direct ingredient), never recursively deeper - keeps
      // Motor->Electromagnet->Iron from self-blocking its own parent's gate.
...
// Mirrors EnsureFeasible's effectiveTarget so BuildPlan can reserve exactly that, after the fact.
...
// Reserves on-hand stock for cumulative queue-support demand; idempotent via _supportReserved
// (only the new delta each call). Existing queued output and output planned this cycle satisfy
// the requirement BEFORE any physical stock is touched.
...
// Recurses for the FULL aspirational amount so shortages surface for future cycles, but gates
// what is committed this cycle by budget - AddQueueItem is never issued for an infeasible amount.
void EnsureFeasible(string alias, double demandIncrement, bool isSupportBranch, HashSet<string> stack, int depth)
{
  ...
  // Support demand can't be satisfied by stock inside this alias's own [Stock] floor
  // (a CopperWire=5000 floor must still trigger +300 for a 300 support need).
  ...
  // Support reservation happens ONLY in BuildPlan, never here, or a raw ingredient could
  // zero its budget before its parent's gate reads it.
  PRec rec;
  ...
  stack.Add(alias);
  // Recurse for the FULL amount first so deep shortages surface for future cycles.
  for (int i = 0; i < recipe.Inputs.Count; i++)
    EnsureFeasible(Canon(recipe.Inputs[i].Item), recipe.Inputs[i].Amount * jobsWanted, isSupportBranch, stack, depth + 1);
  stack.Remove(alias);
  // Feasibility gate: limited by the ingredient supporting fewest jobs. Not-yet-produced
  // output never counts - multi-cycle convergence.
...
// [BlueprintOverrides] from Custom Data ALWAYS wins; the built-in catalog is consulted
// only when no override exists for the alias.
...
// Apply plan - THE ONLY QUEUE MUTATION IN THIS SCRIPT IS AddQueueItem(), and it only ever
// ADDS a newly computed positive job count. Nothing clears/removes/reorders a queue entry.
void ApplyPlan()
{
...
// Knowledge base. Manufactured-vs-terminal is recipe-driven (Polymer is an Ingot with a Recipe).
void BuildKnowledgeBase()
{
  ...
  // Built-in blueprint catalog - the only source besides [BlueprintOverrides], which always
  // wins (see TryGetBlueprint). Recipe-managed aliases:
  ...
  // KNOWLEDGE ONLY - name->blueprint id lookups for items IOPM does not manufacture. They
  // have no Recipe and no ItemDef, so they can never be planned, queued, or put in [Stock].
  ...
  // only this table makes an item manufacturable.
...
// Diagnostics. The script owns [IOPM.*] ONLY: Custom Data is parsed, [IOPM.*] deleted and
// rewritten, result written back - every other section (user or config) survives verbatim.
...
void WriteDiagnostics()
{
  ...
  // Counters stay scoped to [Stock] roots; every plan record still gets a section.
  ...
  foreach (var kv in _machineHealth)
  {
    ...
    // "Stalled" is reserved for StallCycles reaching the threshold; below that it's Waiting.
  ...
  if (!string.Equals(finalText, _lastWrittenCustomData, StringComparison.Ordinal))
  {
    ...
    // Re-read rather than trust finalText, so the no-self-trigger guarantee survives any
    // normalization. A user edit that arrived mid-cycle is still remembered by _cfgDirty.
    _lastCustomDataSeen = Me.CustomData;
  }
}
...
// [IOPM-Stock] display (Item/Qty/Quota/Queued) - read-only, independent of ProductionEnabled.
class StockRow
{
  public string Alias = "";
...
{
  _stockLastError = "";
  // Row fields precomputed ONCE outside the comparator; failures caught HERE, per item.
...
// Ensures every recipe-managed alias has a [Stock] entry defaulted to 0. Never overwrites a
// user quota, never adds knowledge-only names. Runs inside WriteDiagnostics' single
// read-merge-write pass, sharing its no-self-trigger guarantee.
