// IO Production Manager v2.4.38 - warehouse sorting, production planning, stall recovery,
...
// v2.4.38: Gunpowder OUTPUT YIELD is 10, not 1 - live-proven by one manual blueprint run.
// Still not operational: its blueprint id has never been captured and was not invented.
const string VERSION = "2.4.38";
...
void BuildKnowledgeBase() {
  ...
  AddRecipes(new string[] {
    ...
    // NO BLUEPRINT ID IS KNOWN FOR THIS RECIPE, and none was invented. TryGetBlueprint will
    // fail and the row will report Action=UnknownBlueprint - a loud, designed diagnostic
    // rather than a silent stall. That is deliberately better than guessing an id: a
    // fabricated MyDefinitionId parses fine, fails CanUseBlueprint on every machine, and
    // reports NoMachine instead, which points at the hardware rather than at the catalog.
    // To close it without a code change the player can set, in Custom Data:
    //     [BlueprintOverrides]
    //     Gunpowder=<the real blueprint subtype id>
    // which takes precedence over built-in blueprint knowledge by design.
