    ...
    for (int g = 0; g < groupKeys.Count && transferBudget > 0; g++)
    {
        ...
        foreach (var itemKv in perItemTotal)
        {
            ...
            for (int k = 0; k < srcItems.Count; k++)
            {
                ...
                bool ok = TryTransferItem(eligible[overIdx].Inventory, eligible[underIdx].Inventory, srcItems[k], amt, out moved, out hadRoom);
                if (ok)
                {
                    transferBudget--;
                ...
                {
                    Warn("Unreachable balance destination: " + eligible[underIdx].Block.CustomName);
                }
                break;
            }
        }
...
void EnsureFeasible(string alias, double demandIncrement, bool isSupportBranch, HashSet<string> stack, int depth)
{
    ...
    // Existing/manual queue support reserves the physical stock it needs
    // BEFORE checking whether shortage <= 0 (spec 2.0.1 #3): a demand that is
    // already fully satisfied by on-hand stock must still take that stock out
    // of _budget, or a later automatic [Stock] branch could spend the exact
    // material this queue is depending on. This never touches the queue
    // itself — it only marks physical stock as spoken-for.
    if (isSupportBranch) ReserveSupportBudget(alias, effectiveTarget);
