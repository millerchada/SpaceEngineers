// IO Production Manager v2.4.40 - warehouse sorting, production planning, stall recovery,
// dock-aware logistics, capacity alerts, status + Custom-Data diagnostics. History: CHANGELOG.md.
...
// PHASE ORDER: AlertEvaluation sits between ApplyPlan and WriteDiagnostics. That placement is
// deliberate - alerts observe the SETTLED state of a cycle (sorting, docking and planning have
// all run), and being downstream of ApplyPlan makes it structurally impossible for an alert to
// become a queue-management side effect. It is also upstream of WriteDiagnostics so the same
// cycle reports what it decided.
const int PHASE_IDLE = 0, PHASE_DISCOVER = 1, PHASE_SORTING = 2, PHASE_SCAN = 3,
  ...
  PHASE_BUILDPLAN = 8, PHASE_APPLYPLAN = 9, PHASE_ALERTS = 10, PHASE_DIAGNOSTICS = 11,
  PHASE_STATUSRENDER = 12, PHASE_STOCKRENDER = 13;
static readonly string[] PN = new string[] { "Idle", "Discover", "Sorting", "ScanInventory",
  ...
  "AlertEvaluation", "WriteDiagnostics", "StatusRender", "StockRender" };
...
class Config {
  ...
  // ALERTS. Nothing here duplicates a native Broadcast Controller setting: target, UseAntenna
  // and the chat Custom Name stay on the block, where the player set them.
  public bool AlertsEnabled = false;
  public string AlertController = "Broadcast Controller IOPM";
...
void LoadConfig() {
  ...
  c.AlertsEnabled = _ini.Get("Alerts", "Enabled").ToBoolean(false);
  c.AlertController = _ini.Get("Alerts", "BroadcastController").ToString("Broadcast Controller IOPM").Trim();
  c.AlertWarnPct = Math.Max(0, Math.Min(100, _ini.Get("Alerts", "WarehouseWarningPercent").ToDouble(85)));
  c.AlertCritPct = Math.Max(0, Math.Min(100, _ini.Get("Alerts", "WarehouseCriticalPercent").ToDouble(95)));
  ...
  // = value", which meant Computer=500 alongside BasicComputer=2000 silently collapsed to
  // whichever GetKeys happened to return LAST. The text pass preserved both lines and warned,
  // but the ACTIVE QUOTA was still one value picked by iteration order - a number the player
  // never chose, driving real production. Now a colliding alias gets NO entry at all: it is
  // absent from StockTargets, so it is not a planner root, has no demand, is not counted in
  // StockItems and does not appear on the LCD. A conflicting quota is not a quota.
  // Equal values collide identically: the rule is about the CONFLICT, not the numbers, so
  // Computer=500 with BasicComputer=500 is still refused rather than quietly accepted -
  // otherwise the same config would be valid or invalid depending on a value the player may be
  // about to edit.
  // Seeding is unaffected: EnsureStockAliasesPresent tests presence against the [Stock] KEYS,
  // not against StockTargets, so a collided alias still counts as present and no extra key is
  // added on top of the conflict.
  _ini.GetKeys("Stock", keys);
  for (int i = 0; i < keys.Count; i++) {
    string rawName = keys[i].Name;
    if (rawName != null && !_items.ContainsKey(Canon(rawName)))
      _configErrors.Add("Unknown [Stock] item: " + rawName);
  }
  List<string> sOrder = new List<string>();
  Dictionary<string, List<string>> sGroups = GroupStockKeys(_ini, sOrder);
  for (int i = 0; i < sOrder.Count; i++) {
    string alias = sOrder[i];
    List<string> g = sGroups[alias];
    if (g.Count > 1) { _configErrors.Add(StockCollisionMessage(alias, g)); continue; }
    double value;
    if (!double.TryParse(_ini.Get("Stock", g[0]).ToString(), out value) || value < 0) {
      _configErrors.Add("Invalid stock target: " + g[0]);
      continue;
    }
...
void Discover() {
  ...
  for (int i = 0; i < all.Count; i++) {
    ...
    // ALERT TRANSPORT DISCOVERY. Folded into the one same-construct enumeration that already
    // runs, so it costs a string compare per block and no second GetBlocks pass. Matching the
    // CONFIGURED EXACT NAME (case-insensitively) rather than by block type is what makes the
    // selection deterministic - and counting every match rather than taking the first is what
    // lets duplicates FAIL CLOSED instead of silently picking one by enumeration order.
    // A Broadcast Controller and a radio antenna both have no inventory, so this must sit
    // ABOVE the HasInventory gate below.
    if (string.Equals(b.CustomName, _cfg.AlertController, OIC)) {
      _bcFound++;
      if (_bcBlock == null) _bcBlock = b;
    }
    // Antenna state is read ONLY to describe transport health. IOPM never selects an antenna,
...
void BuildKnowledgeBase() {
  ...
  AddItemGroup("MyObjectBuilder_Ingot", "Polymer,IronIngot=Iron,NickelIngot=Nickel,CobaltIngot=Cobalt,CopperIngot=Copper,GoldIngot=Gold,AluminumIngot=Aluminum,TitaniumIngot=Titanium,SilverIngot=Silver,SiliconWafer=Silicon,Carbon,Sulfur,LithiumPaste=Lithium,TantalumIngot=Tantalum,PlatinumIngot=Platinum,PotassiumNitrate=Niter," +
    // v2.4.35 DEPENDENCY IDENTITIES, live-resolved. Recipe INPUTS, never [Stock] roots - they
    // ride the group's stockConfigurable=false, so [Stock] stays at 45 products.
    //   Gravel    -> MyObjectBuilder_Ingot/Stone
    //   Gunpowder -> MyObjectBuilder_Ingot/Magnesium
    // GUNPOWDER IS THE INTERESTING ONE. Industrial Overhaul reuses the VANILLA Magnesium
    // subtype for what the UI calls Gunpowder, which is why a dump search for "Gunpowder"
    // found nothing while the ingot warehouse visibly held a large stack: the dump reported
    // MyObjectBuilder_Ingot/Magnesium=196,886. Naming the alias Gunpowder rather than
    // Magnesium keeps the catalog speaking the game's language, and there is deliberately only
    // ONE alias for the identity - see the loadout table below, where the now-redundant bare
    // Magnesium entry was removed so raw -> alias attribution stays one-to-one. _typeAlias
    // maps MyObjectBuilder_Ingot/Magnesium to exactly one alias, so ScanQueues and _onHand
    // attribution remain deterministic.
    "Gravel=Stone,Gunpowder=Magnesium", false);
  // SyntheticFabric is COMPONENT-typed, so it cannot ride the component group above - that one
  // is stockConfigurable=true and would push [Stock] to 46. It is a recipe input, not a
  // product anyone wants a base floor for, so it gets its own call with false.
  // Note the ordering dependency: this runs BEFORE AddBlueprintGroup, so by the time the
  // knowledge table's "Fabric=Fabric" entry is read, Canon("Fabric") already resolves to
  // SyntheticFabric and the blueprint re-keys itself onto the alias. That is what lets
  // BlueprintReverseMap attribute a MANUALLY queued Fabric. Moving this call after the
  // blueprint groups would silently break that.
  AddItemGroup("MyObjectBuilder_Component", "SyntheticFabric=Fabric", false);
  ...
  AddBlueprintGroup("Electromagnet=Electromagnet,CopperWire=CopperWire,Motor=POMotorComponent,HeatingElement=HeatingElement,SteelPlate=POSteelPlate,SmallSteelTube=POSmallTube,AdvancedComputer=AdvancedComputer,Plastic=PolymerToPlastic,Construction=POConstructionComponent,LargeSteelTube=POLargeTube,BasicComputer=POComputerComponent,Rubber=Rubber,TitaniumPlate=TitaniumPlate,Ceramic=Ceramic,Polymer=SyntheticPolymer,Lightbulb=Lightbulb,Display=PODisplay,MedicalComponent=POMedicalComponent,Thermocouple=Thermocouple,LithiumPowerCell=POPowerCell,GoldWire=GoldWire,AluminumPlate=POInteriorPlate,Glass=POBulletproofGlass,MetalGrid=POMetalGrid,SensorCluster=PODetectorComponent," +
    // v2.4.39, captured live by IO_Blueprint_Sniffer v1.0.3 on the Munitions Factory:
    //   MyObjectBuilder_BlueprintDefinition/Gunpowder
    // The subtype happens to equal the alias, which is exactly why it was never guessed -
    // several ids in this table do NOT follow that pattern (POSteelPlate, POMotorComponent,
    // PODetectorComponent), so "the obvious one" would have been a coin flip. Adding it
    // here rather than to the knowledge-only group because Gunpowder has an active recipe:
    // TryGetBlueprint is now reached from EnsureFeasible and ApplyPlan, not just from
    // BlueprintReverseMap.
    "Gunpowder=Gunpowder");
  ...
  AddLoadAlias("MyObjectBuilder_Component", "Girder=Girder,RadioCommunication=RadioCommunication," +
    ...
    "GravityGenerator=GravityGenerator,Explosives=Explosives,Canvas=Canvas," +
    // "Detector=Detector" and "BulletproofGlass=BulletproofGlass" were removed in v2.4.35 for
    // the same reason as Magnesium: both are BARE SUBTYPES of identities ItemDefs already own
    // (SensorCluster -> Component/Detector, Glass -> Component/BulletproofGlass), and both are
    // registered as aliases twice over - once automatically by AddItem, once explicitly in
    // AddAliasGroup. A loadout line "Detector" still resolves at priority A. The *Component
    // spellings below are NOT redundant: they cannot be derived from a subtype.
    "RadioCommComponent=RadioCommunication," +
  ...
  AddLoadAlias("MyObjectBuilder_Ingot", "DepletedUranium=DepletedUranium,FuelOil=FuelOil," +
    "PrototechScrap=PrototechScrap,SpentFuel=SpentFuel,Uranium=Uranium");
  // "Magnesium=Magnesium" WAS HERE and was removed in v2.4.35. The identity now has an ItemDef
  // (Gunpowder), which ResolveLoadoutType consults at priority A - ahead of this table - and
  // AddItem registers the bare SubtypeId as an alias, so a loadout line "Magnesium" still
  // resolves, through Canon -> Gunpowder -> MyObjectBuilder_Ingot/Magnesium. Keeping both
  // would have been two independent names for one physical identity, which is exactly the
  // ambiguity that makes raw -> alias attribution non-deterministic.
  // MANAGED RECIPE SET - 47 aliases. 37 through v2.4.29; ArmoredPlate in v2.4.30; Capacitor,
  // Girder, RadioCommunication and SolarCell in v2.4.34; Concrete, Explosives and Canvas in
  // v2.4.35; SyntheticFabric in v2.4.36.
  // DEPENDENCY CLOSURE IS COMPLETE as of v2.4.37: every leaf beneath every stock root is an
  // explicit process boundary. That is NOT the same as the catalog being complete - see
  // tests/audit_machine_coverage.py, which compares against live per-machine evidence and
  // still reports 70+ products the mod exposes that IOPM does not model. Counting roots hid
  // SyntheticFabric and Gunpowder for three versions; counting closure hides everything the
  // graph never references.
  ...
  AddRecipes(new string[] {
    ...
    // v2.4.34, live IO 1.7.7 evidence. Four of the seven recipe-pending products. Every
    // ingredient below already resolves to an existing ItemDef, and every blueprint id is
    // already in the table below - nothing new had to be added to make these queueable, and
    // no alternate alias was created.
    //   SiliconWafer is the ItemDef alias for MyObjectBuilder_Ingot/Silicon; SolarCell uses
    //   that existing alias rather than introducing a second name for the same item.
    // OUTPUT YIELD IS 1 for all four: ordinary single-result components with no evidence of a
    // multi-output recipe. Lightbulb|10 is NOT a precedent to generalise from - it is a
    // measured non-unity yield (30 Lightbulbs consumed exactly 3 Glass, UAT-confirmed).
    // MACHINE TOKEN: the producing machine was not observed for any of these four, so the
    // generic "Assembler" token is used. This is a PREFERENCE only - it orders machines that
    // are already eligible. Eligibility is decided by CanUseBlueprint(), so a machine the
    // token does not name still receives the job when it is the only one that can build it,
    // and MachineRank falls back to 5000 for everything unmatched. A wrong preference costs
    ...
    // IronIngot:3, NOT :2. The v2.4.34 recipe was built from an operator reading of 2; the
    // consolidated IO 1.7.7 machine enumeration reads 3 and supersedes it. Caught by
    // audit_machine_coverage comparing the shipped recipe against live evidence - the first
    // defect any audit has found in a recipe that was already running.
    "SolarCell|1|Assembler|Glass:1,CopperWire:1,SiliconWafer:5,IronIngot:3",
    // v2.4.35 - the last three. Recipe-pending set is now EMPTY.
    // Canvas is the game's Parachute recipe; its machine token is the only one in this whole
    // catalog backed by DIRECT observation of the producing blocks rather than inference, so
    // it names both. Output 1: nothing in the repo or the tests ever established a non-unity
    // yield for it, and inventory stack size is not evidence of yield.
    ...
    // v2.4.36. SyntheticFabric was a DEPENDENCY leaf, never a stock root, which is exactly why
    ...
    // OUTPUT YIELD 10, NOT 1. Live-proven: one manual blueprint execution in the Munitions
    // Factory placed 10 Gunpowder in the machine output. v2.4.37 shipped this as yield 1
    // because the UI does not display a yield for it - and DEFAULTING TO 1 IS A GUESS, not a
    // neutral choice. The error is not cosmetic: EnsureFeasible computes
    //     jobsWanted = ceil(shortage / recipeOutput)
    // so a demand for 10 Gunpowder under yield 1 asks for TEN jobs and reserves 60/20/20
    // ingredients, ten times what the recipe needs, then produces 100. Under the correct
    // yield 10 it asks for ONE job and 6/2/2. ScanQueues multiplies queued jobs by the same
    // figure, so a wrong yield also mis-credits anything queued by hand.
    // This is the second measured non-unity yield in the catalog after Lightbulb|10. Treat an
    // absent UI yield as UNKNOWN and measure it, never as 1.
    // Live-observed on the Munitions Factory at 0.8s. Identity was already known: IO reuses
    // the vanilla MyObjectBuilder_Ingot/Magnesium subtype for the item its UI calls Gunpowder,
    // and PotassiumNitrate -> MyObjectBuilder_Ingot/Niter is now live-confirmed too
    // (Niter=62,016.97 in the same dump that showed Magnesium=196,894.34).
    // FULLY EVIDENCED AND OPERATIONAL as of v2.4.39. Every fact below was measured, not
    // inferred, and each took a different method to establish:
    //   identity  MyObjectBuilder_Ingot/Magnesium   Item Identity Dump - found by quantity,
    //             because IO reuses the vanilla Magnesium subtype and a search for the
    //             display name "Gunpowder" returns nothing
    //   producer  Munitions Factory                 machine enumeration
    //   inputs    PotassiumNitrate 6, Carbon 2, Sulfur 2   machine enumeration
    //   yield     10                                one manual blueprint run, counted - the
    //             UI displays no yield, and an absent yield is UNKNOWN, never 1
    //   blueprint MyObjectBuilder_BlueprintDefinition/Gunpowder   IO_Blueprint_Sniffer
    // BlueprintReverseMap iterates _items and Gunpowder is an ItemDef, so with the blueprint
    // id present a MANUALLY queued Gunpowder job is now attributed to the alias and credited
    // at jobs x 10 by ScanQueues.
...
bool _bcWorking, _bcComponent, _bcUseAntenna;
string _alTransport = "Disabled";
bool _alActive = false;
...
string _alLastMsg = "";
// Classify a fill percent, biased towards the level already held. Hysteresis applies only on
// the way DOWN: a band is entered the instant its threshold is crossed, and left only once the
// reading has fallen hyst below it. So with warn=85 hyst=2, 85.0 -> 84.9 is still Warning.
static int AlertLevel(int cur, double pct, double warn, double crit, double hyst) {
  if (pct >= crit) return AL_CRIT;
  if (cur == AL_CRIT && pct >= crit - hyst) return AL_CRIT;
  if (pct >= warn) return AL_WARN;
...
// An ESCALATION is never suppressed. A warehouse crossing into Critical is the message that
// matters most, and holding it back to satisfy a noise budget would be exactly backwards.
string AlertStep(string key, string band, string label, double pct, double warn, double crit, double hyst, double cooldown, double dt, bool silent) {
  ASt st;
  if (!_alerts.TryGetValue(key, out st)) { st = new ASt(); _alerts[key] = st; }
  st.Since += dt;
...
void AlertResolve() {
  ...
  _bcComponent = false;
  _bcUseAntenna = false;
  _bcWorking = _bcFound == 1 && _bcBlock != null && _bcBlock.IsWorking;
  if (_bcWorking) {
    // The chat API lives on an entity COMPONENT, not on the block interface: the ingame
    // IMyBroadcastControllerBlock is a marker interface with no members of its own. A missing
    // component is a diagnostic, not a crash, so the lookup is guarded.
    try {
      IMyChatBroadcastControllerComponent ch;
      if (_bcBlock.Components.TryGet(out ch) && ch != null) {
        _bcChat = ch;
        _bcComponent = true;
        _bcUseAntenna = ch.UseAntenna; // READ ONLY. Never written: it is the player's setting.
      }
...
void AlertEvaluation() {
  ...
  for (int i = 0; i < CATS.Length; i++) {
    ...
    PoolStats(pool, out healthy, out pct); // the SAME capacity arithmetic the diagnostics and
...
// [Stock] ALIAS COLLISIONS. A collision is two or more keys that canonicalize to the same item
// - "Computer=500" alongside "BasicComputer=1000". Re-spelling both to BasicComputer would
// collapse them and silently destroy one player-entered number, so a colliding GROUP is never
// renamed: every key in it is emitted verbatim and the conflict is reported instead. The player
// decides which to keep, because only the player knows which number they meant.
// _stockCollide holds the canonical keys that must not be renamed; _stockCollisions holds the
// human-readable messages. Both are rebuilt every cycle from the parsed section.
HashSet<string> _stockCollide = HS();
List<string> _stockCollisions = new List<string>();
void WriteDiagnostics() {
  ...
  // ALERTS. Concise on purpose: the character budget is real, and a historical alert log in
  // Custom Data would grow without bound. Enabled= is always emitted so "ran and stayed quiet"
  // is distinguishable from "was never asked to run".
  string alk = "IOPM.Alerts";
  ini.Set(alk, "Enabled", _cfg.AlertsEnabled);
  ini.Set(alk, "Controller", _cfg.AlertController);
  ...
  ini.Set(alk, "SendErrors", _alSendErrors);
  ini.Set(alk, "LastMessage", _alLastMsg == "" ? "none" : _alLastMsg);
  ini.Set(alk, "States", AlertStateSummary());
  ...
  for (int i = 0; i < _stockCollisions.Count; i++)
    ini.Set("IOPM.ConfigError." + (_configErrors.Count + i + 1), "Message", _stockCollisions[i]);
...
string CanonicalizeStock(MyIni ini, string text) {
  ...
  for (int i = 0; i < keys.Count; i++) {
    ...
    // "BasicComputer=500" and the section really is canonical rather than merely sorted as if
    // it were. The VALUE is carried across as raw text and is never touched.
    // EXCEPT in a collision, where two keys mean the same item: renaming would collapse them
    // and destroy a player-entered number. A colliding group keeps every original spelling, is
    // reported through [IOPM.ConfigError.*], and is left for the player to resolve. That also
...
// Groups the [Stock] keys by canonical alias and flags any group holding more than one key.
// Runs BEFORE the IOPM.* sections are written so the messages reach [IOPM.ConfigError.*] in the
// same cycle they are detected. Seeding cannot itself create a collision - it only adds a key
// when no existing key canonicalizes to it - so this only ever fires on hand-edited config.
// Groups the KNOWN [Stock] keys by canonical alias. Shared by LoadConfig and the text pass so
// the two can never disagree about what counts as a collision - one deciding to fail closed
// while the other renames would be the worst of both. Groups and their contents are sorted, so
// everything downstream is a pure function of the key SET, never of its iteration order.
Dictionary<string, List<string>> GroupStockKeys(MyIni ini, List<string> order) {
  Dictionary<string, List<string>> groups = new Dictionary<string, List<string>>(SCI);
  List<MyIniKey> gk = new List<MyIniKey>();
  ini.GetKeys("Stock", gk);
  for (int i = 0; i < gk.Count; i++) {
    string n = gk[i].Name;
    if (n == null) continue;
    string c = Canon(n);
    if (!_items.ContainsKey(c)) continue; // unknown keys own their own name; cannot collide
    List<string> g;
    if (!groups.TryGetValue(c, out g)) { g = new List<string>(); groups[c] = g; if (order != null) order.Add(c); }
    g.Add(n);
  }
  if (order != null) order.Sort((a, b) => string.Compare(a, b, OIC));
  foreach (var kv in groups) kv.Value.Sort((a, b) => string.Compare(a, b, OIC));
  return groups;
}
string StockCollisionMessage(string alias, List<string> g) {
  return "[Stock] alias collision on " + alias + ": " + string.Join(", ", g.ToArray()) +
    " all mean " + alias + ". NO quota is applied for " + alias + " until this is resolved - " +
    "all keys are kept verbatim and none renamed. Delete whichever you do not want.";
}
void DetectStockCollisions(MyIni ini) {
  _stockCollide.Clear();
  _stockCollisions.Clear();
  // Ordering and message text come from the shared helper, so this pass and LoadConfig always
  // agree. The message must not depend on the order keys happen to appear in: this pass
  // reorders the section, so a discovery-order message churned for one extra cycle (caught by
  // tests_canonicalize_stock.py before it shipped).
  List<string> order = new List<string>();
  Dictionary<string, List<string>> groups = GroupStockKeys(ini, order);
  for (int i = 0; i < order.Count; i++) {
    List<string> g = groups[order[i]];
    if (g.Count < 2) continue;
    _stockCollide.Add(order[i]);
    _stockCollisions.Add(StockCollisionMessage(order[i], g));
  }
}
