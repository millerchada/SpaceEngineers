// IO Production Manager v2.4.3 - warehouse sorting, production planning, stall recovery,
...
// v2.4.3: PB-whitelist fix (no named comparison-delegate type) + live-verified ItemDef
// subtypes (AluminumPlate/SensorCluster/Glass) + SpaceCredit->Misc. No logic changes.
...
void WriteDiagnostics() {
  ...
  ini.Set(og, "LastAttempt", _orgLastContainer + " | " + _orgLastItem + " | slot " +
    _orgLastSourceIndex + "->" + _orgLastTargetIndex + " | " + (_orgLastResult ? "ok" : "fail"));
...
void RenderStatusScreens() {
  WriteSurfaces(_statScr, BuildStatusText());
}
...
void RenderStockScreens() {
  ...
  WriteSurfaces(_stockScr, BuildStockText());
}
