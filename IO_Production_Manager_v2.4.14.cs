// IO Production Manager v2.4.14 - warehouse sorting, production planning, stall recovery,
...
// v2.4.14: the PB's own surface is ALWAYS written, not only as a fallback - otherwise it sits
// frozen on stale status (and a stale warning) forever once a [StatusScreen] block is tagged.
const string VERSION = "2.4.14";
...
void Discover() {
  ...
  // The PB's own surface is ALWAYS a status target, never only a fallback. Adding it only when
  // no tagged screen existed left it frozen on whatever was last rendered - including a stale
  // "no [StatusScreen]" warning - for as long as a tagged screen stayed present.
  _fallbackScr = _statScr.Count == 0;
  IMyTextSurface own = Me.GetSurface(0);
  if (own != null && !_statScr.Contains(own)) _statScr.Add(own);
...
void BuildKnowledgeBase() {
  ...
  AddItemGroup("MyObjectBuilder_Ingot", "Polymer,IronIngot=Iron,NickelIngot=Nickel,CobaltIngot=Cobalt,CopperIngot=Copper,GoldIngot=Gold,AluminumIngot=Aluminum,TitaniumIngot=Titanium,SilverIngot=Silver,SiliconWafer=Silicon,Carbon,Sulfur,LithiumPaste=Lithium,TantalumIngot=Tantalum,PlatinumIngot=Platinum");
